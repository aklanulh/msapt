...
# Railway PostgreSQL variables - check for PostgreSQL service
if [ -n "$PGHOST" ]; then
    ...
    echo "Using alternative PostgreSQL variables..."
    ...
    echo "No Railway PostgreSQL variables found, using Railway defaults..."
    echo "DB_HOST=${DB_HOST:-postgres.railway.internal}" >> .env
...
# Skip SQLite setup since we're using PostgreSQL
