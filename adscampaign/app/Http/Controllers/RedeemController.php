...
class RedeemController extends Controller
{
    ...
    {
        // Ambil semua data penarikan dari database untuk pengguna yang sedang login
        $user = auth()->user();
        $redeemHistory = Redeem::where('user_id', $user->id)->get();

        // Kirim data penarikan ke view
        return view('redeems.index', compact('redeemHistory'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'amount' => 'required|numeric|min:50', // Ubah nilai minimal menjadi 50
        ], [
            'amount.min' => 'The amount must be at least 50 points.', // Pesan kustom untuk validasi minimal
        ]);
